...
if __name__ == "__main__":
    ...
    # Create dataloaders - use local data if available, otherwise use original path
    data_path = os.path.join(os.getcwd(), "sub-Han_desc-train_behavior+ecephys.nwb")
    if os.path.exists(data_path):
        # Use local data files
        data_dir = os.getcwd()
        print(f"Using local sub-Han data from: {data_dir}")
    else:
        # Fall back to original path
        data_dir = os.path.join(args.home, "datasets/000127/sub-Han/")
        print(f"Using original data path: {data_dir}")
    
    (train_dataset, test_dataset,
     train_dl, test_dl) = make_monkey_reaching_dataset(data_dir,
                                                       mode=args.mode,
